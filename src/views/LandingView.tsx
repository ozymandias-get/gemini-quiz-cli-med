...
import { ArrowRight, Brain, Sparkles, Zap, Microscope, Copy, LibraryBig, Play, Terminal, RefreshCw, History, Trash2 } from 'lucide-react';
...
import { selectLanguage, selectSetLanguage } from '../store';
import { useResumeQuizState } from '../hooks/useResumeQuizState';
import { useGeminiCliPanel } from '../hooks/useGeminiCliPanel';
...
export const LandingView: FC = () => {
  ...
  const { canResumeQuiz, handleResumeQuiz, handleDeleteResumeQuiz } = useResumeQuizState();
  const {
    cliStatus,
    cliLoading,
    checkCliStatus,
      ...
      <section className="flex flex-col items-center text-center pt-0 md:pt-2 px-4 relative z-10">
        ...
        >
          {canResumeQuiz && (
            <div className="flex items-stretch gap-3">
              <Button
                ...
                className="flex-1 border-2 border-emerald-400/80 bg-emerald-900/40 py-4 text-base font-bold !text-emerald-100 shadow-md shadow-emerald-900/25 ring-1 ring-emerald-400/35 hover:border-emerald-300 hover:bg-emerald-900/55 hover:!text-white dark:border-emerald-500/60 dark:bg-emerald-950/55 dark:!text-emerald-100 dark:ring-emerald-500/30 dark:hover:border-emerald-300 dark:hover:bg-emerald-900/65 dark:hover:!text-white sm:min-w-[200px] md:text-lg"
              >
              ...
              <button
                type="button"
                onClick={handleDeleteResumeQuiz}
                aria-label={t.landing.deleteResumeQuizAria}
                title={t.landing.deleteResumeQuizAria}
                className="inline-flex min-w-12 items-center justify-center rounded-xl border-2 border-red-400/70 bg-red-900/35 px-3 text-red-100 shadow-md shadow-red-900/20 ring-1 ring-red-400/30 transition-colors hover:border-red-300 hover:bg-red-900/55 hover:text-white dark:border-red-500/60 dark:bg-red-950/50 dark:text-red-100 dark:ring-red-500/30 dark:hover:border-red-300 dark:hover:bg-red-900/65"
              >
                <Trash2 size={18} />
              </button>
            </div>
          ...
          <Button
            ...
            className="border-2 border-stone-300/70 bg-stone-900/45 py-4 text-base font-extrabold !text-stone-100 shadow-lg shadow-black/20 ring-1 ring-stone-300/30 hover:border-stone-200 hover:bg-stone-900/65 hover:!text-white dark:border-stone-500/60 dark:bg-stone-900/60 dark:!text-stone-100 dark:ring-stone-500/30 dark:hover:border-stone-300 dark:hover:bg-stone-800/70 dark:hover:!text-white sm:min-w-[200px] md:text-lg"
          >
