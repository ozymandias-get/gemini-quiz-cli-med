...
import { useResultsFilter, type ResultsFilterType, getQuestionStatus } from '../hooks/useResultsFilter';
...
export const ResultsView: FC = () => {
  ...
  const quizState = useQuizSessionStore(selectQuizState);
  const settings = useSettingsStore(selectSettings);
  const fileName = useGenerationStore(selectFileName);
  const pdfSourcePath = useGenerationStore(selectPdfSourcePath);
  const preparedDocument = useGenerationStore((state) => state.preparedDocument);
  const [filter, setFilter] = useState<ResultsFilterType>('ALL');
  ...
  const { hasMistakes, filteredQuestions } = useResultsFilter(quizState, filter);
  const filterValues = useMemo<ResultsFilterType[]>(() => ['ALL', 'WRONG', 'EMPTY'], []);
  ...
  return (
    <div className="max-w-4xl mx-auto w-full pb-20 px-4 relative">
      ...
      <AnimatePresence>
        {selectedQuestion && preparedDocument && (
            <SourceViewerModal 
      ...
      >
        ...
        <div className="bg-white/40 dark:bg-stone-900/40 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/50 dark:border-stone-800 shadow-sm flex flex-wrap gap-2 md:gap-3 justify-center md:justify-start">
             {quizState.questions.map((q, idx) => {
                 const status = getQuestionStatus(quizState, q.id, q.correctAnswerIndex);
        ...
        <AnimatePresence initial={false}>
            {filteredQuestions.length > 0 ? (
                ...
                >
                    ...
                    isCorrect={quizState.userAnswers[question.id] === question.correctAnswerIndex} 
                    onShowSource={pdfSourcePath || preparedDocument ? (question) => setSelectedQuestion(question) : undefined}
                    />
                </motion.div>
                ))
            ) : (
