...
export const SettingsModal: FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const showCliWarning = inTauri && cliStatus.isChecked && (!cliStatus.isInstalled || !cliStatus.isHeadlessReady);
        ...
        {showCliWarning && (
          <div className="mb-6 rounded-2xl border border-amber-200/90 bg-amber-50/90 p-4 text-left dark:border-amber-800/50 dark:bg-amber-950/40">
            ...
            {cliStatus.statusMessage ? (
              <p className="mt-2 text-xs leading-snug text-amber-800 dark:text-amber-200">{cliStatus.statusMessage}</p>
            ) : null}
            <button
              type="button"
