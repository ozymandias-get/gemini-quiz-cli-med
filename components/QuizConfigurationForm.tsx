...
export const QuizConfigurationForm: FC = () => {
  ...
  return (
    <motion.div variants={configItemVariants} className={`${configPanel3dWrapClass} h-full min-h-0 max-h-full`}>
      ...
      >
        <div
          className="absolute top-0 right-0 w-64 h-64 bg-sand-200/20 dark:bg-sand-500/5 rounded-full blur-[80px] pointer-events-none"
          style={{ zIndex: 0 }}
        />
        <div
          className="absolute bottom-0 left-0 w-48 h-48 bg-amber-200/10 dark:bg-indigo-500/5 rounded-full blur-[60px] pointer-events-none"
          style={{ zIndex: 0 }}
        />

        <div className="flex items-center justify-between pb-2 md:pb-3 lg:pb-2 shrink-0 relative z-10">
          ...
          <div className="p-2 bg-white/50 dark:bg-stone-800/50 rounded-full shadow-sm ring-1 ring-black/5 dark:ring-white/10">
            <Wand2 size={18} className="text-sand-600 dark:text-sand-400" />
          </div>
        </div>
        ...
        <div
          className="flex-1 min-h-0 min-w-0 overflow-y-auto overscroll-contain pr-1 space-y-2.5 md:space-y-3 lg:space-y-2 relative z-10 scrollbar-thin py-0.5 lg:py-0 [scrollbar-gutter:stable]"
